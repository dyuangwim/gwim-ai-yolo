...
class ResultCard(QtWidgets.QFrame):
    ...
    def __init__(self, info, parent=None):
        ...
        img_frame = QtWidgets.QFrame(); img_frame.setStyleSheet("background:#020617; border-radius:10px;")
        img_layout = QtWidgets.QVBoxLayout(img_frame); img_layout.setContentsMargins(4,4,4,4)
        self.imageLabel = QtWidgets.QLabel(); self.imageLabel.setAlignment(QtCore.Qt.AlignCenter); self.imageLabel.setMinimumSize(220,120)
        self.imageLabel.setStyleSheet("background:#020617;border-radius:8px;color:#64748b;")
        img_layout.addWidget(self.imageLabel); root.addWidget(img_frame,1)
        ...
        self.timeLabel = QtWidgets.QLabel(ts_text); self.timeLabel.setStyleSheet("color:#0f172a;font-weight:bold;"); bl.addWidget(self.timeLabel)
        expected = info.get("expected",0); ng = info.get("ng_count",0); pc = info.get("pack_count",0)
        self.summaryLabel = QtWidgets.QLabel(f"Detected: {expected} batteries/pkg | Packs: {pc} | NG: {ng}" if pc else f"NG packs: {ng}")
        self.summaryLabel.setStyleSheet("color:#0f172a;"); bl.addWidget(self.summaryLabel); root.addWidget(bottom,0)
    ...
    def set_image(self, pix): size=self.imageLabel.size() or QtCore.QSize(220,120); scaled=pix.scaled(size, QtCore.Qt.KeepAspectRatio, QtCore.Qt.SmoothTransformation); self.imageLabel.setPixmap(scaled)
...
class DetailDialog(QtWidgets.QDialog):
    def __init__(self, info, parent=None):
        super().__init__(parent)
        self.setWindowTitle("Inspection Details"); self.resize(520,520); self.setModal(True)
        ...
        ts_label = QtWidgets.QLabel(ts_text); ts_label.setStyleSheet("color:#6b7280;"); lay.addWidget(ts_label)
        ...
        img = QtWidgets.QLabel(); img.setAlignment(QtCore.Qt.AlignCenter); img.setMinimumSize(400,260); img.setStyleSheet("background:#0b1120;border-radius:10px;"); il.addWidget(img); lay.addWidget(imgf)
        if info.get("image_path") and os.path.exists(info["image_path"]):
        ...
        exp = info.get("expected",0); ng = info.get("ng_count",0); pc = info.get("pack_count",0)
        for i,(k,v) in enumerate([("Expected per pack", f"{exp} batteries"),("Total packs", str(pc)),("NG packs", str(ng))]):
            lbl=QtWidgets.QLabel(k); lbl.setStyleSheet("color:#6b7280;"); val=QtWidgets.QLabel(v); val.setStyleSheet("font-weight:bold;"+("color:#22c55e;" if k=="NG packs" and ng==0 else ""))
            grid.addWidget(lbl,0,i); grid.addWidget(val,1,i)
        lay.addWidget(inf)
        box = QtWidgets.QFrame(); box.setStyleSheet("background:%s;border-radius:12px;" % ("#dcfce7" if ng==0 else "#fee2e2"))
        bl = QtWidgets.QVBoxLayout(box); bl.setContentsMargins(12,8,12,8)
        title = QtWidgets.QLabel("Status"); title.setStyleSheet("font-weight:bold;"); bl.addWidget(title)
...
class LogReaderThread(QtCore.QThread):
    ...
    def run(self):
        proc = self._process; current_image=None; json_path=None
        while True:
            ...
            if line.startswith("Image: "):
                ...
                current_image=None; json_path=None
        proc.wait(); self.processExited.emit(proc.returncode)
    def _build_info(self, image_path, json_path):
        if not image_path: return None
        info = {"image_path": image_path, "expected": self._expected, "ts": datetime.now(),
                "ng_count":0, "pack_count":0, "packs":[], "json_path": json_path}
        if json_path and os.path.exists(json_path):
            try:
                with open(json_path,"r") as f: data=json.load(f)
                packs=data.get("packs",[]); info["packs"]=packs; info["pack_count"]=len(packs)
...
class MainWindow(QtWidgets.QMainWindow):
    def __init__(self):
        ...
        for lbl in [self.label_stat_expected,self.label_stat_total,self.label_stat_passed,self.label_stat_failed]:
            lbl.setStyleSheet("color:#6b7280;")
        for lbl in [self.stat_expected_val,self.stat_total_val,self.stat_passed_val,self.stat_failed_val]:
            lbl.setStyleSheet("font-size:20px;font-weight:bold;")
        for lbl in [self.label_stat_expected_unit,self.label_stat_total_unit,self.label_stat_passed_unit,self.label_stat_failed_unit]:
    ...
    def update_expected_stat(self):
        expected = int(self.combo_expected.currentData()); self.stat_expected_val.setText(str(expected))
    ...
    def start_inspection(self):
        ...
        self.btn_start.setEnabled(False); self.combo_expected.setEnabled(False); self.btn_stop.setEnabled(True)
    ...
    def on_new_result(self, info):
        ...
        if info.get("ng_count",0)>0:
            self.warning_frame.setVisible(True); self.btn_stop_alarm.setEnabled(True)
        else:
            self.warning_frame.setVisible(False)
