...
def make_variants(bgr):
    """降低分辨率目标到 ~720，精简为 5 个变体，覆盖锐化/二值/反相/强锐化/180°。"""
    ...
    outs = [
        cv2.cvtColor(sharp,  cv2.COLOR_GRAY2RGB),
        cv2.cvtColor(otsu,   cv2.COLOR_GRAY2RGB),
        cv2.cvtColor(inv,    cv2.COLOR_GRAY2RGB),
        cv2.cvtColor(strong, cv2.COLOR_GRAY2RGB),
    ]
    # 只补一个 180° 旋转
    outs.append(cv2.rotate(outs[0], cv2.ROTATE_180))
    return outs
...
# ========= OCR 实现（RapidOCR 优先、Tesseract 备选；单例）=========
_RAPID = None
def _get_rapid():
    global _RAPID
    if _RAPID is None:
...
def rapidocr_read(imgs)->List[Tuple[str,float,str]]:
    """加入‘早停’：命中白名单且 conf>=0.55 就停止后续变体。"""
    ocr = _get_rapid()
    if not ocr:
        return []
    outs=[]
    for i,im in enumerate(imgs):
        try:
            ...
            text = " ".join(all_text)
            outs.append((text, best_conf, f"v{i:02d}"))

            # 早停
            if extract_models(_normalize_text(text)) and best_conf >= 0.55:
                break
        except Exception:
            pass
    return outs
...
def choose_best(results, expected: Optional[str]=None):
    ...
    for txt, conf, _ in results:
        ...
        if not models:
            ...
            bonus = 0.5 if (expected and expected in models) else 0.0
